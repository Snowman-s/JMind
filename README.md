...
- forの最後の行、(現在は267行目)の部分にnopが挟まっているのは、StackMapTableの整合性をとるためです。いずれ修正します。
- 変数宣言が今の仕様のままだと難しいかもしれないです。それは、forが変数の枠をとってしまっているためです。
# 細かい仕様
...
- ならば～つぎに - スタックから数値を一つ取り出し、それが１（真）に等しかったなら内部が実行されます。
  - 所謂if文に相当します。
- でなければ～つぎに - スタックから数値を一つ取り出し、それが０（偽）に等しかったなら内部が実行されます。
